...
def calc_lane_states_only_ones(l_center, l_heading, l_width, v_width, d, nxy=1):
    """
    calc lane states
    :param l_center: lane lateral position
    :param l_heading:  lane heading
    ...
    states = []
    delta = -0.5 * (l_width - v_width) + (l_width - v_width)
    xf = xc - delta * math.sin(l_heading)
    yf = yc + delta * math.cos(l_heading)
    yawf = l_heading
    states.append([xf, yf, yawf])
...
def sampling_test_bo():
    ...
    print("before calc")
    states = calc_lane_states_only_ones(l_center, l_heading, l_width, v_width, d)
    print("after calc, before generate_path")
    result = generate_path(states, k0)
